...
public class DemoModule1CardinalityExample {
    ...
    public static void main(String[] args) {
        ...
        Single<String> firstGreekLetterOnly = Observable.fromArray(GreekAlphabet.getGreekLetters())
                .first("?");

        // Maybe result stream - First letter (alpha)
        Maybe<String> maybeGreekLetterOneEvent = Observable.fromArray(GreekAlphabet.getGreekLetters())
                .first("?")
                .filter( nextLetter -> nextLetter.equals("\u03b1"));

        // Maybe result stream - No letters
        Maybe<String> maybeGreekLetterNoEvents = Observable.fromArray(GreekAlphabet.getGreekLetters())
                .first("?")
                .filter( nextLetter -> !nextLetter.equals("\u03b1"));

        // Completable result stream - No output to subscriber, only success or failure.
        Completable completableObserver = Observable.fromArray(GreekAlphabet.getGreekLetters())
                .ignoreElements();
